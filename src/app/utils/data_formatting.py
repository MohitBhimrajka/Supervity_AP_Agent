    ...
    if invoice.status == models.DocumentStatus.needs_review and invoice.exception_details:
        for exc in invoice.exception_details:
            ...
            formatted_exc = {
                ...
                "details": [{"label": "Details", "value": exc.get('message', 'No details provided')}]
            }
...
def format_full_dossier(invoice: models.Invoice, db: Session) -> dict:
    ...
    dossier = {
        ...
        "documents": {
            ...
            "grn": {
                "data": schemas.GoodsReceiptNote.from_orm(grn).model_dump(mode='json') if grn else None,
                "file_path": grn.file_path if grn else None
            },
            "po": {
                "data": schemas.PurchaseOrder.from_orm(po).model_dump(mode='json') if po else None,
