...
import { Loader2, CheckCircle, AlertTriangle, FileText, Magnet, ShieldCheck } from "lucide-react";
...
const Step = ({ icon: Icon, title, status }: { icon: React.ElementType, title: string, status: 'active' | 'completed' | 'pending' }) => (
    <div className="flex flex-col items-center gap-2">
        <div className={cn(
            ...
            status === 'completed' && "bg-green-success border-green-success text-white",
            status === 'active' && "bg-blue-primary border-blue-primary text-white animate-pulse",
...
export const JobProgress = ({ initialJob, onComplete }: JobProgressProps) => {
  ...
  const getOverallStatusInfo = () => {
    switch (job.status) {
        ...
        default:
            return { icon: <Loader2 className="w-5 h-5 animate-spin" />, text: "In Progress...", variant: "default" as const };
    }
  };
  ...
  return (
    <div className="p-4 border rounded-lg bg-gray-50 flex flex-col h-full">
        ...
        <div className="flex justify-around items-center my-auto">
            <Step icon={FileText} title="Ingestion" status={job.status === 'processing' ? 'active' : 'completed'}/>
            <div className={cn("flex-grow h-1 mx-4 rounded-full", job.status !== 'processing' ? 'bg-green-success' : 'bg-gray-200')} />
            <Step icon={Magnet} title="Matching" status={job.status === 'matching' ? 'active' : (job.status === 'processing' ? 'pending' : 'completed')} />
            <div className={cn("flex-grow h-1 mx-4 rounded-full", isFinished ? 'bg-green-success' : 'bg-gray-200')} />
            <Step icon={ShieldCheck} title="Finalized" status={isFinished ? 'completed' : 'pending'} />
        </div>

        <div className="mt-auto">
            <div className="w-full bg-gray-200 rounded-full h-2.5">
                <div className="bg-blue-primary h-2.5 rounded-full transition-all duration-500" style={{ width: `${progress}%` }}></div>
            </div>
