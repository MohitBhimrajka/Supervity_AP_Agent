  ...
  if (jobs.length === 0) {
    return (
      <div className="text-center text-gray-800 py-8">
        ...
        <p className="text-sm mt-1">Upload your first documents in the Document Hub to see processing history.</p>
      </div>
    );
  }
      ...
      <TableBody>
        {jobs.map((job) => (
          <TableRow key={job.id}>
            ...
            <TableCell>{format(new Date(job.created_at), 'MMM d, yyyy h:mm a')}</TableCell>
          </TableRow>
        ))}
      </TableBody>
    </Table>
