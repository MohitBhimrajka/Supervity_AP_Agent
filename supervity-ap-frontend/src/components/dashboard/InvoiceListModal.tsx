        ...
        <TableBody>
          {invoices.length > 0 ? invoices.map(inv => (
            <TableRow key={inv.id}>
              ...
              <TableCell className="text-right">${inv.grand_total?.toFixed(2) ?? '0.00'}</TableCell>
...
export const InvoiceListModal = ({ isOpen, onClose, title, children }: InvoiceListModalProps) => {
  return (
