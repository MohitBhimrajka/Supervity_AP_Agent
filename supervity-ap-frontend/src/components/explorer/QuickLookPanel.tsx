...
export const QuickLookPanel = ({ invoiceDbId }: QuickLookPanelProps) => {
    ...
    if (error) {
        return <div className="p-4 text-center text-pink-destructive"><AlertTriangle className="mx-auto w-8 h-8 mb-2" />{error}</div>;
    }
            ...
            <div className="flex-grow space-y-6">
                <div className="pb-4 border-b">
                    <div className="flex justify-between items-start">
                        <div>
                            <h3 className="text-xl font-bold text-black">{data.invoice_id}</h3>
                            <p className="text-gray-500">From: {data.vendor_name}</p>
                        </div>
                        <Badge variant={data.invoice_status === 'matched' ? 'success' : 'warning'}>
                            {data.invoice_status.replace(/_/g, ' ')}
                        </Badge>
                    </div>
                    <p className="text-3xl font-bold mt-2">${(data.grand_total ?? 0).toFixed(2)}</p>
                </div>
                
                <div>
                    <h4 className="font-semibold text-lg mb-2">Exception Summary</h4>
                    <ExceptionSummary trace={data.match_trace} />
                </div>
            </div>
            
            <div className="mt-auto pt-4 border-t">
                <Link href={`/resolution-workbench?invoiceId=${data.invoice_id}`} className="w-full">
