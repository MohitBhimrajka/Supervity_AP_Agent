...
styles.add(ParagraphStyle(name='DocTitle', fontSize=22, fontName='Helvetica-Bold', alignment=1))
...
def draw_po_pdf(po_data, filename):
    ...
    story.append(Paragraph(po_data['buyer_address'].replace('\n', '<br/>'), styles['Normal']))
    ...
    # PO Info Table
    info_data = [
        [Paragraph("<b>VENDOR</b>", styles['Normal']), Paragraph("<b>SHIP TO</b>", styles['Normal'])],
        [Paragraph(f"{po_data['vendor_name']}<br/>{po_data['vendor_address'].replace('\n', '<br/>')}", styles['Normal']),
         Paragraph(f"{po_data['buyer_name']}<br/>{po_data['buyer_address'].replace('\n', '<br/>')}", styles['Normal'])],
        ['', ''], # Spacer row
        [Paragraph(f"<b>PO NUMBER:</b><br/>{po_data['po_number']}", styles['Normal']),
         Paragraph(f"<b>ORDER DATE:</b><br/>{po_data['order_date'].strftime('%B %d, %Y')}", styles['Normal'])]
    ]
    ...
    item_table = Table(item_data, colWidths=[0.8*inch, 2.5*inch, 0.7*inch, 0.7*inch, 1*inch, 1.3*inch])
    ...
    totals_table = Table(totals_data, colWidths=[0.8*inch, 2.5*inch, 0.7*inch, 0.7*inch, 1*inch, 1.3*inch])
    ...
    story.append(Spacer(1, 0.2*inch))
    story.append(Paragraph("<i>Note: This PO total represents the expected total value of all invoices. Multiple shipments and invoices may be issued against this PO.</i>", styles['Normal']))
...
def draw_grn_pdf(po_data, grn_number, received_date, received_items, filename):
    ...
    story.append(Paragraph(f"INTERNAL GOODS RECEIPT NOTE (GRN) - {po_data['buyer_name']}", styles['Heading1']))
    story.append(Spacer(1, 0.3*inch))
    
    story.append(Paragraph(f"<b>GRN Number:</b> {grn_number}", styles['Normal']))
    story.append(Paragraph(f"<b>Received Date:</b> {received_date.strftime('%Y-%m-%d')}", styles['Normal']))
    story.append(Paragraph(f"<b>Reference PO:</b> {po_data['po_number']}", styles['Normal']))
    story.append(Paragraph(f"<b>Vendor:</b> {po_data['vendor_name']}", styles['Normal']))
    story.append(Spacer(1, 0.3*inch))

    story.append(Paragraph("<b>Items Received in this Shipment:</b>", styles['Heading3']))
    
    item_data = [['SKU', 'DESCRIPTION', 'QTY RECEIVED', 'UNIT']]
    for item in received_items:
        item_data.append([item['sku'], item['description'], item['received_qty'], item['unit']])
    ...
    story.append(Paragraph("Signature: _________________________<br/>Printed Name: _____________________<br/>Dept: Warehouse/Receiving", styles['Normal']))
    
    doc.build(story)

def draw_invoice_pdf(invoice_data, invoice_number, invoice_date, due_date, filename):
    """Generates the Vendor Invoice PDF for a specific shipment."""
    doc = SimpleDocTemplate(filename, pagesize=letter)
    story = []

    # Vendor Header (ArcelorMittal)
    story.append(Paragraph(invoice_data['vendor_name'], styles['VendorHeader']))
    story.append(Paragraph(invoice_data['vendor_address'].replace('\n', '<br/>'), styles['Normal']))
    ...
    info_data = [
        [Paragraph(f"<b>BILL TO:</b><br/>{invoice_data['buyer_name']}<br/>{invoice_data['buyer_address'].replace('\n', '<br/>')}", styles['Normal']),
         Paragraph(f"<b>Invoice #:</b> {invoice_number}<br/>"
                   f"<b>Date:</b> {invoice_date.strftime('%m/%d/%Y')}<br/>"
                   f"<b>Due Date:</b> {due_date.strftime('%m/%d/%Y')}", styles['RightAlign'])]
    ]
    story.append(Table(info_data, colWidths=[3.5*inch, 3*inch]))
    story.append(Spacer(1, 0.1*inch))
    story.append(Paragraph(f"<b>Reference PO: {invoice_data.get('po_number', 'N/A')}</b>", styles['Normal']))
    story.append(Paragraph(f"<b>Reference GRN: {invoice_data.get('grn_number', 'N/A')}</b>", styles['Normal']))
    story.append(Spacer(1, 0.3*inch))
    
    item_data = [['DESCRIPTION', 'QTY', 'UNIT PRICE', 'AMOUNT']]
    for item in invoice_data['line_items']:
        item_data.append([item['description'], item['billed_qty'], f"${item['unit_price']:,.2f}", f"${item['total']:,.2f}"])
    
    item_table = Table(item_data, colWidths=[3.5*inch, 0.5*inch, inch, 1.5*inch])
    item_table.setStyle(TableStyle([('ALIGN', (1,0), (-1,-1), 'RIGHT'), ('LINEBELOW', (0,0), (-1,0), 2, colors.black),
...
def draw_po_md(po_data, filename):
    ...
    content.append(f"{po_data['vendor_address'].replace(chr(10), '  \n')}\n") # Add Markdown line breaks
    
    content.append(f"\n**Ship To / Buyer:**\n")
    content.append(f"{po_data['buyer_name']}\n")
    content.append(f"{po_data['buyer_address'].replace(chr(10), '  \n')}\n")
    ...
    # Table Rows
    for item in po_data['line_items']:
        content.append(f"| {item['sku']} | {item['description']} | {item['ordered_qty']} | {item['unit']} | ${item['unit_price']:,.2f} | ${item['line_total']:,.2f} |")
    ...
    content.append("---\n")
    content.append("*Note: This PO total represents the expected total value of all invoices. Multiple shipments and invoices may be issued against this PO.*")
...
def draw_grn_md(po_data, grn_number, received_date, received_items, filename):
    ...
    content.append("|:----|:------------|:------------:|:-----|")
    for item in received_items:
        content.append(f"| {item['sku']} | {item['description']} | {item['received_qty']} | {item['unit']} |")
...
def draw_invoice_md(invoice_data, invoice_number, invoice_date, due_date, filename):
    ...
    content.append(f"{invoice_data['buyer_address'].replace(chr(10), '  \n')}\n")
    ...
    for item in invoice_data['line_items']:
        ...
        subtotal += line_total
        content.append(f"| {item['description']} | {item['billed_qty']} | ${item['unit_price']:,.2f} | ${line_total:,.2f} |")
